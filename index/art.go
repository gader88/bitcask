...
func (art *AdaptiveRadixTree) Put(key []byte, pos *data.LogRecordsPos) *data.LogRecordsPos {
	art.lock.Lock()
	...
	return olderIt.(*data.LogRecordsPos)
}
func (art *AdaptiveRadixTree) Get(key []byte) *data.LogRecordsPos {
	art.lock.RLock()
	...
	return value.(*data.LogRecordsPos)
}
func (art *AdaptiveRadixTree) Delete(key []byte) (*data.LogRecordsPos, bool) {
	art.lock.Lock()
	...
	return olderIt.(*data.LogRecordsPos), del
}
...
func newArtIterator(tree art.Tree, reverse bool) *artIterator {
	...
	saveValues := func(node art.Node) bool {
		item := &Item{
			key: node.Key(),
			pos: node.Value().(*data.LogRecordsPos),
		}
		values[idx] = item
		if reverse {
			idx--
...
func (art *artIterator) Value() *data.LogRecordsPos {
